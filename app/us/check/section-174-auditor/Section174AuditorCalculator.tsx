...
export default function Section174AuditorCalculator() {
  ...
  async function handleBracketSelect(index: number) {
    ...
    sessionStorage.setItem("section-174-auditor_bracket", b.label);
    sessionStorage.setItem("section-174-auditor_status",  b.status);
    ...
    try {
      ...
      if (data.id) {
        ...
        localStorage.setItem("section-174-auditor_session_id", data.id);
      }
    } catch { /* non-blocking */ }
  }
  ...
  async function handleSaveEmail() {
    ...
    fetch("/api/save-email", {
      ...
      body: JSON.stringify({
        email,
        source:       "section-174-auditor_result",
        country_code: "US",
        site:         "taxchecknow",
        session_id:   sessionId ?? localStorage.getItem("section-174-auditor_session_id") ?? "",
      }),
    }).catch(() => {});
    setEmailSent(true);
  }
      ...
      if (sid) {
        fetch("/api/decision-sessions", {
          ...
          body: JSON.stringify({
            ...
            product_key:           `us_${effectiveTier}_section_174_auditor`,
          ...
          <div ref={resultRef} className={`rounded-2xl border p-6 sm:p-8 ${statusStyle.panelClass}`}>
            ...
            <div className="mt-3 rounded-xl border border-neutral-200 bg-white px-4 py-3">
              ...
              <ul className="space-y-1 text-xs text-neutral-700">
                <li className="flex items-start gap-2"><span className="mt-0.5 text-emerald-600 shrink-0">✓</span><span>Your Section 174 Exposure Report — Your exact year-1 deduction and phantom income under current law.</span></li>
                <li className="flex items-start gap-2"><span className="mt-0.5 text-emerald-600 shrink-0">✓</span><span>Expense Reclassification Guide — Costs that may not qualify as R&E and can be expensed immediately.</span></li>
                <li className="flex items-start gap-2"><span className="mt-0.5 text-emerald-600 shrink-0">✓</span><span>Offshore vs Domestic Cost Allocation — How to correctly allocate and document US vs foreign R&E costs.</span></li>
                <li className="flex items-start gap-2"><span className="mt-0.5 text-emerald-600 shrink-0">✓</span><span>Section 41 R&D Credit Analysis — Can the research tax credit offset your Section 174 timing impact?</span></li>
                <li className="flex items-start gap-2"><span className="mt-0.5 text-emerald-600 shrink-0">✓</span><span>CPA Brief — Section 174 — Questions for your CPA about Section 174 compliance and planning.</span></li>
              </ul>
            </div>
            ...
            <button
              ...
              Get My Section 174 Exposure Report →
            </button>
