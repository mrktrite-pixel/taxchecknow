...
// Product: iso-amt-sniper · Tier 2 Success Page
...
const FILES = [
  {
    "num": "01",
    "slug": "iso-amt-01",
    "name": "Your Safe Exercise Range",
    "desc": "Your exact safe exercise number — how many ISOs you can exercise before triggering AMT.",
    "tier": 1,
    "start": false
  },
  {
    "num": "02",
    "slug": "iso-amt-02",
    "name": "Your 409A Timing Strategy",
    "desc": "How the 409A valuation affects your AMT exposure — and when to exercise.",
    "tier": 1,
    "start": true
  },
  {
    "num": "03",
    "slug": "iso-amt-03",
    "name": "AMT Credit Tracker (Form 8801)",
    "desc": "How to recover AMT paid on ISO exercise in future years.",
    "tier": 1,
    "start": false
  },
  {
    "num": "04",
    "slug": "iso-amt-04",
    "name": "Disqualifying Disposition Strategy",
    "desc": "When to sell early to reverse AMT — and the tax cost of doing so.",
    "tier": 1,
    "start": false
  },
  {
    ...
    "desc": "Print and take to your next meeting before exercising any ISOs.",
    "tier": 1,
    "start": false
  },
  {
    "num": "06",
    "slug": "iso-amt-06",
    "name": "Dual Basis Tracker",
    "desc": "Regular tax basis vs AMT basis — why they differ and why it matters.",
    "tier": 2,
    "start": false
  },
  {
    "num": "07",
    "slug": "iso-amt-07",
    "name": "State Tax Overlay (California Focus)",
    "desc": "California and other states that do not recognise ISO tax treatment.",
    "tier": 2,
    "start": false
  },
  {
    "num": "08",
    "slug": "iso-amt-08",
    "name": "Multi-Year Exercise Plan",
    "desc": "How to exercise a large grant across multiple years without triggering AMT.",
    "tier": 2,
    "start": false
  }
];
...
interface Assessment {
  ...
  safeExerciseRange: string | string[] | { title: string; deadline: string; steps: string[] }[];
  amtExposure: string | string[] | { title: string; deadline: string; steps: string[] }[];
  liquidityRisk: string | string[] | { title: string; deadline: string; steps: string[] }[];
  creditRecovery: string | string[] | { title: string; deadline: string; steps: string[] }[];
  dualBasis: string | string[] | { title: string; deadline: string; steps: string[] }[];
...
export default function SuccessPlan() {
  ...
  async function generateAssessment(name: string) {
    setLoading(true);
    try {
      const iso_salary = sessionStorage.getItem("iso-amt-sniper_iso_salary") || "150000";
...
Their data:
- Annual salary: ${iso_salary}\n- ISOs considering: ${iso_shares}\n- Spread per share: ${iso_spread}\n- AMT status: ${iso_status}
...
"status": "...", "safeExerciseRange": "...", "amtExposure": "...", "liquidityRisk": "...", "creditRecovery": "...", "dualBasis": "...", "actions": "...", "weekPlan": "...", "accountantQuestions": "..."
...
For array fields use actual arrays. For action items use objects with title, deadline, steps array.`;
  ...
  function buildFallback(name: string): Assessment {
    ...
    result["safeExerciseRange"] = "safeExerciseRange — building for " + displayName;
    result["amtExposure"] = "amtExposure — building for " + displayName;
    result["liquidityRisk"] = "liquidityRisk — building for " + displayName;
    result["creditRecovery"] = "creditRecovery — building for " + displayName;
    result["dualBasis"] = "dualBasis — building for " + displayName;
    result["actions"] = [];
    result["weekPlan"] = [];
  ...
  function handleCalendar() {
    ...
    const ics = [
      ...
      "CALSCALE:GREGORIAN",
      "METHOD:PUBLISH",
      `X-WR-CALNAME:ISO AMT Exercise Sniper — Action Dates`,
      "BEGIN:VEVENT",
      `UID:iso-plan-${Date.now()}@taxchecknow.com`,
      `DTSTART;VALUE=DATE:${relativeDate(7)}`,
      `DTEND;VALUE=DATE:${relativeDate(7)}`,
      `DTSTAMP:${now}`,
      "SUMMARY:ISO AMT — Plan your safe exercise range",
      "DESCRIPTION:Calculate safe exercise range with CPA before any exercise.",
      "STATUS:CONFIRMED",
      "END:VEVENT",
      "BEGIN:VEVENT",
      `UID:iso-409a-${Date.now()}@taxchecknow.com`,
      `DTSTART;VALUE=DATE:${relativeDate(14)}`,
      `DTEND;VALUE=DATE:${relativeDate(14)}`,
      `DTSTAMP:${now}`,
      "SUMMARY:ISO AMT — Monitor 409A valuation update",
      "DESCRIPTION:Exercise before next 409A if company has had funding event.",
      "STATUS:CONFIRMED",
      "END:VEVENT",
      "BEGIN:VEVENT",
      `UID:iso-q2-${Date.now()}@taxchecknow.com`,
      `DTSTART;VALUE=DATE:${"20260616"}`,
      `DTEND;VALUE=DATE:${"20260616"}`,
      `DTSTAMP:${now}`,
      "SUMMARY:ISO AMT — Q2 Estimated Tax Due",
      "DESCRIPTION:Include AMT from Q1 exercises in estimated tax payment.",
      "STATUS:CONFIRMED",
      "END:VEVENT",
      "BEGIN:VEVENT",
      `UID:iso-q3-${Date.now()}@taxchecknow.com`,
      `DTSTART;VALUE=DATE:${"20260915"}`,
      `DTEND;VALUE=DATE:${"20260915"}`,
      `DTSTAMP:${now}`,
      "SUMMARY:ISO AMT — Q3 Estimated Tax Due",
      "DESCRIPTION:Q3 estimated tax payment.",
      "STATUS:CONFIRMED",
      "END:VEVENT",
      "BEGIN:VEVENT",
      `UID:iso-year-${Date.now()}@taxchecknow.com`,
      `DTSTART;VALUE=DATE:${"20261231"}`,
      `DTEND;VALUE=DATE:${"20261231"}`,
      `DTSTAMP:${now}`,
      "SUMMARY:🔴 ISO AMT — Year-End Exercise Deadline",
      "DESCRIPTION:Last day to exercise for 2026 tax year. Do not exercise without calculating safe range.",
      "STATUS:CONFIRMED",
      "END:VEVENT",
      "BEGIN:VEVENT",
      `UID:iso-file-${Date.now()}@taxchecknow.com`,
      `DTSTART;VALUE=DATE:${"20270415"}`,
      `DTEND;VALUE=DATE:${"20270415"}`,
      `DTSTAMP:${now}`,
      "SUMMARY:ISO AMT — File Form 6251 + Form 8801",
      "DESCRIPTION:AMT calculation (Form 6251) and Minimum Tax Credit (Form 8801) due with annual return.",
      "STATUS:CONFIRMED",
      "END:VEVENT",
      "END:VCALENDAR",
    ].join("\r\n");
    ...
    a.href     = url;
    a.download = "iso-amt-sniper-deadlines.ics";
  ...
  async function handleCopy() {
    ...
    await navigator.clipboard.writeText(
      `ISO AMT Exercise Sniper — questions for my accountant:\n\n${questions}\n\nPrepared by TaxCheckNow · taxchecknow.com`
    );
    setCopied(true);
    ...
    <div className="min-h-screen bg-neutral-50 print:bg-white">
      ...
      <nav className="no-print border-b border-neutral-200 bg-white px-6 py-4">
        <div className="mx-auto flex max-w-3xl items-center justify-between">
          ...
          <span className="font-mono text-xs text-neutral-400">United States · ISO AMT Exercise Sniper</span>
        </div>
      </nav>
      ...
      <main className="mx-auto max-w-3xl space-y-5 px-6 py-8">
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="font-mono text-[10px] uppercase tracking-widest text-emerald-700">
                Payment confirmed · Your AMT Recovery System · £147
              </p>
              <h1 className="mt-1 font-serif text-2xl font-bold text-neutral-950">
                {greeting} ISO AMT Exercise Sniper Action Plan
              </h1>
                ...
                <div className="mb-3 flex items-start justify-between gap-4">
                  <div>
                    ...
                    <p className="mt-0.5 text-xs text-blue-600">
                      Full brief in{" "}
                      <a href="/files/us/iso-amt-sniper/iso-amt-05"
            ...
            <div className="print-section rounded-2xl border border-neutral-200 bg-white p-6">
              ...
              <div className="mb-4 space-y-2">
                ...
                <div className="flex items-center justify-between rounded-xl border border-neutral-100 bg-neutral-50 px-4 py-2.5">
                  <span className="text-sm text-neutral-700">ISO AMT — Plan your safe exercise range</span>
                  <span className="ml-3 shrink-0 font-mono text-xs font-bold text-neutral-500">This week</span>
                </div>
                <div className="flex items-center justify-between rounded-xl border border-neutral-100 bg-neutral-50 px-4 py-2.5">
                  <span className="text-sm text-neutral-700">ISO AMT — Monitor 409A valuation update</span>
                  <span className="ml-3 shrink-0 font-mono text-xs font-bold text-neutral-500">This week</span>
                </div>
                <div className="flex items-center justify-between rounded-xl border border-neutral-100 bg-neutral-50 px-4 py-2.5">
                  <span className="text-sm text-neutral-700">ISO AMT — Q2 Estimated Tax Due</span>
                  <span className="ml-3 shrink-0 font-mono text-xs font-bold text-neutral-500">16 Jun 2026</span>
                </div>
                <div className="flex items-center justify-between rounded-xl border border-neutral-100 bg-neutral-50 px-4 py-2.5">
                  <span className="text-sm text-neutral-700">ISO AMT — Q3 Estimated Tax Due</span>
                  <span className="ml-3 shrink-0 font-mono text-xs font-bold text-neutral-500">15 Sep 2026</span>
                </div>
                <div className="flex items-center justify-between rounded-xl border border-neutral-100 bg-neutral-50 px-4 py-2.5">
                  <span className="text-sm text-neutral-700">🔴 ISO AMT — Year-End Exercise Deadline</span>
                  <span className="ml-3 shrink-0 font-mono text-xs font-bold text-neutral-500">31 Dec 2026</span>
                </div>
                <div className="flex items-center justify-between rounded-xl border border-neutral-100 bg-neutral-50 px-4 py-2.5">
                  <span className="text-sm text-neutral-700">ISO AMT — File Form 6251 + Form 8801</span>
                  <span className="ml-3 shrink-0 font-mono text-xs font-bold text-neutral-500">15 Apr 2027</span>
                </div>
              </div>
              ...
              <div className="space-y-2">
                {FILES.map(f => (
                  <div key={f.num}
                    ...
                    <a
                      href={`/files/us/iso-amt-sniper/${f.slug}`}
                      target="_blank"
        ...
        <div className="rounded-xl bg-neutral-100 px-5 py-4">
          <p className="text-xs leading-relaxed text-neutral-500">
            ...
            <a href="https://www.irs.gov/taxtopics/tc427" target="_blank" rel="noopener noreferrer" className="underline hover:text-neutral-700">IRS — Incentive Stock Options</a> · <a href="https://www.irs.gov/taxtopics/tc556" target="_blank" rel="noopener noreferrer" className="underline hover:text-neutral-700">IRS — Alternative Minimum Tax</a>
          </p>
        </div>
