...
// Product: interest-reinstatement-engine · Tier 2 Success Page
...
const FILES = [
  {
    "num": "01",
    "slug": "ir-01",
    "name": "Your Interest Recovery Calculation",
    "desc": "Your exact annual tax saving from restored interest deductibility.",
    "tier": 1,
    "start": false
  },
  {
    "num": "02",
    "slug": "ir-02",
    "name": "Nexus of Funds Tracker",
    "desc": "Link each borrowing to its rental use — the IRD audit defence tool.",
    "tier": 1,
    "start": true
  },
  {
    "num": "03",
    "slug": "ir-03",
    "name": "Loan Split Optimiser",
    "desc": "Separate deductible rental debt from non-deductible personal debt.",
    "tier": 1,
    "start": false
  },
  {
    "num": "04",
    "slug": "ir-04",
    "name": "Ring-Fenced Loss Guide",
    "desc": "How ring-fencing affects your restored interest deductions.",
    "tier": 1,
    "start": false
  },
  {
    ...
    "desc": "Print and take to your tax return meeting — interest questions your adviser must answer.",
    "tier": 1,
    "start": false
  },
  {
    "num": "06",
    "slug": "ir-06",
    "name": "Debt Shifting Framework",
    "desc": "Legally move debt onto rental properties to maximise deductible interest.",
    "tier": 2,
    "start": false
  },
  {
    "num": "07",
    "slug": "ir-07",
    "name": "LTC vs Personal Ownership Audit",
    "desc": "Does your Look-Through Company structure still make sense with restored deductibility?",
    "tier": 2,
    "start": false
  },
  {
    "num": "08",
    "slug": "ir-08",
    "name": "Multi-Property Optimisation Plan",
    "desc": "Maximise interest deductions across a rental property portfolio.",
    "tier": 2,
    "start": false
  }
];
...
interface Assessment {
  ...
  annualTaxSaving: string | string[] | { title: string; deadline: string; steps: string[] }[];
  cashflowShift: string | string[] | { title: string; deadline: string; steps: string[] }[];
  tracingRisk: string | string[] | { title: string; deadline: string; steps: string[] }[];
  debtShiftOpportunity: string | string[] | { title: string; deadline: string; steps: string[] }[];
  ltcReview: string | string[] | { title: string; deadline: string; steps: string[] }[];
...
"status": "...", "annualTaxSaving": "...", "cashflowShift": "...", "tracingRisk": "...", "debtShiftOpportunity": "...", "ltcReview": "...", "actions": "...", "weekPlan": "...", "accountantQuestions": "..."
...
For array fields use actual arrays. For action items use objects with title, deadline, steps array.`;
  ...
  function buildFallback(name: string): Assessment {
    ...
    result["annualTaxSaving"] = "annualTaxSaving — building for " + displayName;
    result["cashflowShift"] = "cashflowShift — building for " + displayName;
    result["tracingRisk"] = "tracingRisk — building for " + displayName;
    result["debtShiftOpportunity"] = "debtShiftOpportunity — building for " + displayName;
    result["ltcReview"] = "ltcReview — building for " + displayName;
    result["actions"] = [];
    result["weekPlan"] = [];
  ...
  function handleCalendar() {
    ...
    const ics = [
      ...
      "CALSCALE:GREGORIAN",
      "METHOD:PUBLISH",
      `X-WR-CALNAME:Interest Reinstatement Engine — Action Dates`,
      "BEGIN:VEVENT",
      `UID:ir-trace-${Date.now()}@taxchecknow.com`,
      `DTSTART;VALUE=DATE:${relativeDate(14)}`,
      `DTEND;VALUE=DATE:${relativeDate(14)}`,
      `DTSTAMP:${now}`,
      "SUMMARY:Interest — Complete loan tracing",
      "DESCRIPTION:Document the purpose of each loan and any refinanced amounts before filing.",
      "STATUS:CONFIRMED",
      "END:VEVENT",
      "BEGIN:VEVENT",
      `UID:ir-split-${Date.now()}@taxchecknow.com`,
      `DTSTART;VALUE=DATE:${relativeDate(30)}`,
      `DTEND;VALUE=DATE:${relativeDate(30)}`,
      `DTSTAMP:${now}`,
      "SUMMARY:Interest — Split mixed loans",
      "DESCRIPTION:Ask lender to create separate rental and personal loan facilities.",
      "STATUS:CONFIRMED",
      "END:VEVENT",
      "BEGIN:VEVENT",
      ...
      `DTSTAMP:${now}`,
      "SUMMARY:NZ Tax Return — Claim 100% interest",
      "DESCRIPTION:File 2025/26 return with full interest deduction.",
      "STATUS:CONFIRMED",
      "END:VEVENT",
      "BEGIN:VEVENT",
      `UID:ir-yearend-${Date.now()}@taxchecknow.com`,
      `DTSTART;VALUE=DATE:${"20270331"}`,
      `DTEND;VALUE=DATE:${"20270331"}`,
      `DTSTAMP:${now}`,
      "SUMMARY:NZ Tax Year End — 31 March 2027",
      "DESCRIPTION:Plan final interest claims for 2026/27 year.",
      "STATUS:CONFIRMED",
      "END:VEVENT",
      "END:VCALENDAR",
    ].join("\r\n");
    ...
    a.href     = url;
    a.download = "interest-reinstatement-engine-deadlines.ics";
  ...
  async function handleCopy() {
    ...
    await navigator.clipboard.writeText(
      `Interest Reinstatement Engine — questions for my accountant:\n\n${questions}\n\nPrepared by TaxCheckNow · taxchecknow.com`
    );
    setCopied(true);
    ...
    <div className="min-h-screen bg-neutral-50 print:bg-white">
      ...
      <nav className="no-print border-b border-neutral-200 bg-white px-6 py-4">
        <div className="mx-auto flex max-w-3xl items-center justify-between">
          ...
          <span className="font-mono text-xs text-neutral-400">New Zealand · Interest Reinstatement Engine</span>
        </div>
      </nav>
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <h1 className="mt-1 font-serif text-2xl font-bold text-neutral-950">
                {greeting} Interest Reinstatement Engine Action Plan
              </h1>
            ...
            <div className="print-section rounded-2xl border border-neutral-200 bg-white p-6">
              ...
              <div className="mb-4 space-y-2">
                ...
                <div className="flex items-center justify-between rounded-xl border border-neutral-100 bg-neutral-50 px-4 py-2.5">
                  <span className="text-sm text-neutral-700">Interest — Complete loan tracing</span>
                  <span className="ml-3 shrink-0 font-mono text-xs font-bold text-neutral-500">This week</span>
                </div>
                <div className="flex items-center justify-between rounded-xl border border-neutral-100 bg-neutral-50 px-4 py-2.5">
                  <span className="text-sm text-neutral-700">Interest — Split mixed loans</span>
                  <span className="ml-3 shrink-0 font-mono text-xs font-bold text-neutral-500">This week</span>
                </div>
                <div className="flex items-center justify-between rounded-xl border border-neutral-100 bg-neutral-50 px-4 py-2.5">
                  <span className="text-sm text-neutral-700">NZ Tax Return — Claim 100% interest</span>
                  <span className="ml-3 shrink-0 font-mono text-xs font-bold text-neutral-500">7 Oct 2026</span>
                </div>
                <div className="flex items-center justify-between rounded-xl border border-neutral-100 bg-neutral-50 px-4 py-2.5">
              ...
              <div className="space-y-2">
                {FILES.map(f => (
                  <div key={f.num}
                    ...
                    <a
                      href={`/files/nz/interest-reinstatement-engine/${f.slug}`}
                      target="_blank"
