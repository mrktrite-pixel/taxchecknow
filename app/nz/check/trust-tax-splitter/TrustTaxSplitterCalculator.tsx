...
const PRODUCTS: Record<PackTier, { name: string; tagline: string; value: string; cta: string }> = {
  67: {
    ...
    tagline: "Are you paying 39% when you could legally pay 10.5%?",
    value:   "A personalised trust tax audit built around your trust income, your beneficiaries, and your distribution strategy — not a generic trust guide.",
    cta:     "Get My Distribution Pack — $67 →",
  },
  147: {
    name:    "Your Trust vs Company Pivot Audit",
    tagline: "Find the most tax-efficient structure for your trust income",
    value:   "A personalised trust audit plus company comparison, imputation credit mapping, and multi-entity optimisation strategy.",
    cta:     "Get My Pivot Audit — $147 →",
  },
};
...
function recommendedTier(
  ...
  trustIncome: number | string,
  beneficiaryType: number | string,
  hasCompanyComparison: boolean,
): PackTier {
  // trustIncome over $30000 OR beneficiaryType is minors or mixed → tier2. hasCompanyComparison → tier2. Otherwise tier1.
    if ((trustIncome as number) > 30000 || beneficiaryType === "minors" || beneficiaryType === "mixed" || beneficiaryType === "none" || hasCompanyComparison === true) return 147;
...
export default function TrustTaxSplitterCalculator() {
  ...
  const [trustIncome, setTrustIncome] = useState<number | string>(20000);
  const [beneficiaryType, setBeneficiaryType] = useState<number | string>("adults");
  const [hasCompanyComparison, setHasCompanyComparison] = useState<boolean>(false);
  ...
  async function handleBracketSelect(index: number) {
    ...
    setTrustIncome(20000);
    setBeneficiaryType("adults");
    setHasCompanyComparison(false);
    setOverrideTier(null);
    setError("");
    // Save to sessionStorage for success page
    const b = BRACKETS[index];
    sessionStorage.setItem("trust-tax-splitter_bracket", b.label);
    sessionStorage.setItem("trust-tax-splitter_status",  b.status);
    ...
    try {
      ...
      if (data.id) {
        ...
        localStorage.setItem("trust-tax-splitter_session_id", data.id);
      }
    } catch { /* non-blocking */ }
  }
  ...
  async function handleSaveEmail() {
    ...
    fetch("/api/save-email", {
      ...
      body: JSON.stringify({
        email,
        source:       "trust-tax-splitter_result",
        country_code: "NZ",
        site:         "taxchecknow",
        session_id:   sessionId ?? localStorage.getItem("trust-tax-splitter_session_id") ?? "",
      }),
    }).catch(() => {});
    setEmailSent(true);
  }
  ...
  async function handleContinueToPayment() {
    ...
    sessionStorage.setItem("trust-tax-splitter_answers", JSON.stringify(answers));
    sessionStorage.setItem("trust-tax-splitter_trustIncome", String(trustIncome));
    sessionStorage.setItem("trust-tax-splitter_beneficiaryType", String(beneficiaryType));
    sessionStorage.setItem("trust-tax-splitter_hasCompanyComparison", String(hasCompanyComparison));
    const sid          = sessionId ?? localStorage.getItem("trust-tax-splitter_session_id");
      ...
      if (sid) {
        fetch("/api/decision-sessions", {
          ...
          body: JSON.stringify({
            ...
            product_key:           `nz_${effectiveTier}_trust_tax_splitter`,
        ...
        <div className="rounded-2xl border border-neutral-200 bg-white p-6 shadow-sm sm:p-8">
          ...
          <div className="mb-4 rounded-xl border-2 border-neutral-900 bg-neutral-950 px-4 py-4">
            ...
            <p className="mt-1 text-sm leading-relaxed text-white">
              Trustee tax rate: 39%. De minimis: trust income under $10,000 taxed at 33%. Adult beneficiaries (16+): taxed at personal rates (10.5%-33%). Minor beneficiaries (under 16): taxed at 39% — no reduction. Company rate: 28%.
            </p>
          </div>
          ...
          <div className="mt-4 rounded-xl border-2 border-amber-200 bg-amber-50 px-4 py-3">
            ...
            <p className="mt-1 text-xs leading-relaxed text-amber-900">
              Distributions must be made before 31 March to apply in that tax year. If you leave income undistributed, it is taxed at 39% in the trust. You cannot retrospectively distribute income from a prior year to change the tax rate.
            </p>
          </div>
        </div>
          ...
          <div ref={resultRef} className={`rounded-2xl border p-6 sm:p-8 ${statusStyle.panelClass}`}>
            ...
            <div className="rounded-2xl border border-neutral-200 bg-white p-5">
              ...
              <div className="space-y-5">
                {/* BUTTON GROUP — What is the trust's annual net income? */}
                <div>
                  <p className="mb-1 text-sm font-semibold text-neutral-800">What is the trust's annual net income?</p>
                  <p className="mb-2 text-xs text-neutral-500">Net income after allowable deductions</p>
                  <div className="flex flex-wrap gap-2">
                    
                    <button
                      type="button"
                      onClick={() => setTrustIncome(6000)}
                ...
                {/* BUTTON GROUP — Who are the beneficiaries? */}
                <div>
                  <p className="mb-1 text-sm font-semibold text-neutral-800">Who are the beneficiaries?</p>
                  <p className="mb-2 text-xs text-neutral-500">Determines whether distributions reduce tax</p>
                  <div className="flex flex-wrap gap-2">
                    
                    <button
                      type="button"
                      onClick={() => setBeneficiaryType("adults")}
                      className={`rounded-lg border-2 px-4 py-2.5 text-sm font-bold transition ${
                        beneficiaryType === "adults"
                          ? "border-neutral-950 bg-neutral-950 text-white"
                          : "border-neutral-200 bg-white text-neutral-700 hover:border-neutral-950 hover:bg-neutral-950 hover:text-white"
                      }`}>
                      Adults only (16+)
                    ...
                    <button
                      type="button"
                      onClick={() => setBeneficiaryType("mixed")}
                      className={`rounded-lg border-2 px-4 py-2.5 text-sm font-bold transition ${
                    ...
                    <button
                      type="button"
                      onClick={() => setBeneficiaryType("minors")}
                      className={`rounded-lg border-2 px-4 py-2.5 text-sm font-bold transition ${
                        beneficiaryType === "minors"
                          ? "border-neutral-950 bg-neutral-950 text-white"
                          : "border-neutral-200 bg-white text-neutral-700 hover:border-neutral-950 hover:bg-neutral-950 hover:text-white"
                      }`}>
                      Minors only (under 16)
                    </button>
                    <button
                      type="button"
                ...
                {/* TWO-BUTTON TOGGLE — Would you like to compare against a company structure? */}
                <div>
                  <p className="mb-1 text-sm font-semibold text-neutral-800">Would you like to compare against a company structure?</p>
                  <p className="mb-2 text-xs text-neutral-500">Companies pay 28% vs trust trustee rate of 39%</p>
                  <div className="flex gap-2">
                    
                    <button
                      type="button"
                      onClick={() => setHasCompanyComparison(false)}
                      className={`flex-1 rounded-lg border-2 py-3 text-sm font-bold transition ${
                        hasCompanyComparison === false
                          ? "border-neutral-950 bg-neutral-950 text-white"
                          : "border-neutral-200 bg-white text-neutral-700 hover:border-neutral-950 hover:bg-neutral-950 hover:text-white"
                      }`}>
                      No — trust only
                    ...
                    <button
                      type="button"
                      onClick={() => setHasCompanyComparison(true)}
                      className={`flex-1 rounded-lg border-2 py-3 text-sm font-bold transition ${
                        hasCompanyComparison === true
                          ? "border-neutral-950 bg-neutral-950 text-white"
                          : "border-neutral-200 bg-white text-neutral-700 hover:border-neutral-950 hover:bg-neutral-950 hover:text-white"
                      }`}>
                      Yes — compare both
