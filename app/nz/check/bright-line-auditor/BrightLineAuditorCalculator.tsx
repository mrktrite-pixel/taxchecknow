...
const PRODUCTS: Record<PackTier, { name: string; tagline: string; value: string; cta: string }> = {
  67: {
    ...
    tagline: "Do you owe tax on your property sale — yes or no — and can you prove it?",
    value:   "A personalised bright-line audit built around your title date, sale date, and main home usage — not a generic property guide.",
    cta:     "Get My Proof Kit — $67 →",
  },
  147: {
    name:    "Your Rollover Relief Audit",
    tagline: "Protect your exemption during transfers and restructuring",
    value:   "A personalised bright-line audit plus transfer analysis, trust and associated person testing, and rollover eligibility confirmation.",
    cta:     "Get My Rollover Audit — $147 →",
  },
};
...
function recommendedTier(
  ...
  propertyUse: number | string,
  wasTransferred: boolean,
): PackTier {
  // holdingDays under 730 OR wasTransferred → tier2. propertyUse mixed or rental → tier2. Otherwise tier1.
    if ((holdingDays as number) < 730 || propertyUse === "mixed" || propertyUse === "rental" || wasTransferred === true) return 147;
  return 67;
}
...
export default function BrightLineAuditorCalculator() {
  ...
  const [holdingDays, setHoldingDays] = useState<number | string>(548);
  const [propertyUse, setPropertyUse] = useState<number | string>("main_home");
  ...
  async function handleBracketSelect(index: number) {
    ...
    setHoldingDays(548);
    setPropertyUse("main_home");
    setWasTransferred(false);
    setOverrideTier(null);
    setError("");
    // Save to sessionStorage for success page
    const b = BRACKETS[index];
    sessionStorage.setItem("bright-line-auditor_bracket", b.label);
    sessionStorage.setItem("bright-line-auditor_status",  b.status);
    ...
    try {
      ...
      if (data.id) {
        ...
        localStorage.setItem("bright-line-auditor_session_id", data.id);
      }
    } catch { /* non-blocking */ }
  }
  ...
  async function handleSaveEmail() {
    ...
    fetch("/api/save-email", {
      ...
      body: JSON.stringify({
        email,
        source:       "bright-line-auditor_result",
        country_code: "NZ",
        site:         "taxchecknow",
        session_id:   sessionId ?? localStorage.getItem("bright-line-auditor_session_id") ?? "",
      }),
    }).catch(() => {});
    setEmailSent(true);
  }
  ...
  async function handleContinueToPayment() {
    ...
    sessionStorage.setItem("bright-line-auditor_answers", JSON.stringify(answers));
    sessionStorage.setItem("bright-line-auditor_holdingDays", String(holdingDays));
    sessionStorage.setItem("bright-line-auditor_propertyUse", String(propertyUse));
    sessionStorage.setItem("bright-line-auditor_wasTransferred", String(wasTransferred));
    const sid          = sessionId ?? localStorage.getItem("bright-line-auditor_session_id");
      ...
      if (sid) {
        fetch("/api/decision-sessions", {
          ...
          body: JSON.stringify({
            ...
            product_key:           `nz_${effectiveTier}_bright_line_auditor`,
        ...
        <div className="rounded-2xl border border-neutral-200 bg-white p-6 shadow-sm sm:p-8">
          ...
          <div className="mb-4 rounded-xl border-2 border-neutral-900 bg-neutral-950 px-4 py-4">
            ...
            <p className="mt-1 text-sm leading-relaxed text-white">
              Bright-line period: 2 years (730 days) for all residential property sold from 1 July 2024. Start date: title registration. End date: binding sale agreement. Main home exemption: must pass time test (over 50% occupancy) and area test (over 50% of land).
            </p>
          </div>
          ...
          <div className="mt-4 rounded-xl border-2 border-amber-200 bg-amber-50 px-4 py-3">
            ...
            <p className="mt-1 text-xs leading-relaxed text-amber-900">
              Most people are calculating from the wrong date. If you are using your purchase agreement date instead of title registration date, your bright-line calculation may be wrong. Even a few days difference can change your tax position.
            </p>
          </div>
        </div>
          ...
          <div ref={resultRef} className={`rounded-2xl border p-6 sm:p-8 ${statusStyle.panelClass}`}>
            ...
            <div className="rounded-2xl border border-neutral-200 bg-white p-5">
              ...
              <div className="space-y-5">
                {/* BUTTON GROUP — How long did you hold the property (title to sale agreement)? */}
                <div>
                  <p className="mb-1 text-sm font-semibold text-neutral-800">How long did you hold the property (title to sale agreement)?</p>
                  <p className="mb-2 text-xs text-neutral-500">Count from title registration date to binding sale agreement date</p>
                  <div className="flex flex-wrap gap-2">
                    
                    <button
                      type="button"
                      onClick={() => setHoldingDays(180)}
                ...
                {/* BUTTON GROUP — How was the property used? */}
                <div>
                  <p className="mb-1 text-sm font-semibold text-neutral-800">How was the property used?</p>
                  <p className="mb-2 text-xs text-neutral-500">Determines main home exemption eligibility</p>
                  <div className="flex flex-wrap gap-2">
                    
                    <button
                      type="button"
                      onClick={() => setPropertyUse("main_home")}
                      className={`rounded-lg border-2 px-4 py-2.5 text-sm font-bold transition ${
                        propertyUse === "main_home"
                          ? "border-neutral-950 bg-neutral-950 text-white"
                          : "border-neutral-200 bg-white text-neutral-700 hover:border-neutral-950 hover:bg-neutral-950 hover:text-white"
                      }`}>
                      Main home entire time
                    </button>
                    <button
                      type="button"
                      onClick={() => setPropertyUse("mixed")}
                      className={`rounded-lg border-2 px-4 py-2.5 text-sm font-bold transition ${
                        propertyUse === "mixed"
                          ? "border-neutral-950 bg-neutral-950 text-white"
                          : "border-neutral-200 bg-white text-neutral-700 hover:border-neutral-950 hover:bg-neutral-950 hover:text-white"
                      }`}>
                      Mix of main home and rental
                    </button>
                    <button
                      type="button"
                      onClick={() => setPropertyUse("rental")}
                      className={`rounded-lg border-2 px-4 py-2.5 text-sm font-bold transition ${
                        propertyUse === "rental"
                          ? "border-neutral-950 bg-neutral-950 text-white"
                          : "border-neutral-200 bg-white text-neutral-700 hover:border-neutral-950 hover:bg-neutral-950 hover:text-white"
                    ...
                    <button
                      type="button"
                      onClick={() => setPropertyUse("holiday")}
                      className={`rounded-lg border-2 px-4 py-2.5 text-sm font-bold transition ${
                        propertyUse === "holiday"
                          ? "border-neutral-950 bg-neutral-950 text-white"
                          : "border-neutral-200 bg-white text-neutral-700 hover:border-neutral-950 hover:bg-neutral-950 hover:text-white"
                ...
                {/* TWO-BUTTON TOGGLE — Was the property transferred from a trust or family member? */}
                <div>
                  <p className="mb-1 text-sm font-semibold text-neutral-800">Was the property transferred from a trust or family member?</p>
                  <p className="mb-2 text-xs text-neutral-500">Transfers can reset the bright-line clock</p>
                  <div className="flex gap-2">
                    
                    <button
                      type="button"
                      onClick={() => setWasTransferred(false)}
                    ...
                    <button
                      type="button"
                      onClick={() => setWasTransferred(true)}
                      className={`flex-1 rounded-lg border-2 py-3 text-sm font-bold transition ${
                        wasTransferred === true
                          ? "border-neutral-950 bg-neutral-950 text-white"
                          : "border-neutral-200 bg-white text-neutral-700 hover:border-neutral-950 hover:bg-neutral-950 hover:text-white"
