...
const PRODUCTS: Record<PackTier, { name: string; tagline: string; value: string; cta: string }> = {
  67: {
    ...
    tagline: "Are you paying 60% tax without knowing it?",
    value:   "A personalised allowance audit showing your taper exposure, pension relief opportunity, and exact recovery amount.",
    cta:     "Get My Allowance Pack — £67 →",
  },
  147: {
    name:    "Your 60% Tax Escape Plan",
    tagline: "Eliminate the 60% trap with a personalised pension strategy",
    value:   "Full taper audit plus pension contribution modelling, salary sacrifice optimisation, and a written brief for your accountant.",
    cta:     "Get My Escape Plan — £147 →",
  },
};
...
function recommendedTier(
  ...
  adjustedIncome: number | string,
  makesPensionContribs: boolean,
): PackTier {
  // adjustedIncome in taper band (100k-125140) → tier2. No pension contribs → tier2. Otherwise tier1.
    if ((adjustedIncome as number) >= 100000 && (adjustedIncome as number) <= 125140 || makesPensionContribs === true) return 147;
...
export default function AllowanceSniperCalculator() {
  ...
  const [adjustedIncome, setAdjustedIncome] = useState<number | string>(106000);
  const [makesPensionContribs, setMakesPensionContribs] = useState<boolean>(false);
  ...
  async function handleBracketSelect(index: number) {
    ...
    setAdjustedIncome(106000);
    setMakesPensionContribs(false);
    setOverrideTier(null);
    setError("");
    // Save to sessionStorage for success page
    const b = BRACKETS[index];
    sessionStorage.setItem("allowance-sniper_bracket", b.label);
    sessionStorage.setItem("allowance-sniper_status",  b.status);
    ...
    try {
      ...
      if (data.id) {
        ...
        localStorage.setItem("allowance-sniper_session_id", data.id);
      }
    } catch { /* non-blocking */ }
  }
  ...
  async function handleSaveEmail() {
    ...
    fetch("/api/save-email", {
      ...
      body: JSON.stringify({
        email,
        source:       "allowance-sniper_result",
        country_code: "UK",
        site:         "taxchecknow",
        session_id:   sessionId ?? localStorage.getItem("allowance-sniper_session_id") ?? "",
      }),
    }).catch(() => {});
    setEmailSent(true);
  }
  ...
  async function handleContinueToPayment() {
    ...
    sessionStorage.setItem("allowance-sniper_answers", JSON.stringify(answers));
    sessionStorage.setItem("allowance-sniper_adjustedIncome", String(adjustedIncome));
    sessionStorage.setItem("allowance-sniper_makesPensionContribs", String(makesPensionContribs));
    const sid          = sessionId ?? localStorage.getItem("allowance-sniper_session_id");
        ...
        <div className="rounded-2xl border border-neutral-200 bg-white p-6 shadow-sm sm:p-8">
          ...
          <div className="mb-4 rounded-xl border-2 border-neutral-900 bg-neutral-950 px-4 py-4">
            <p className="font-mono text-[10px] uppercase tracking-widest text-neutral-400">
              The taper rule — HMRC confirmed
            </p>
            <p className="mt-1 text-sm leading-relaxed text-white">
              Personal allowance taper: £1 allowance lost for every £2 of adjusted net income above £100,000. Full allowance (£12,570) lost at £125,140. Effective marginal tax rate in taper band: 60%. Pension contributions reduce adjusted net income and restore allowance at up to 60% effective relief.
            </p>
          </div>
          ...
          <div className="mt-4 rounded-xl border-2 border-amber-200 bg-amber-50 px-4 py-3">
            ...
            <p className="mt-1 text-xs leading-relaxed text-amber-900">
              Adjusted net income includes salary, dividends, rental profit, and other taxable income — but is reduced by pension contributions, gift aid donations, and certain other reliefs. Your P60 gross pay is NOT the same as your adjusted net income if you have other income sources.
            </p>
          </div>
        </div>
          ...
          <div ref={resultRef} className={`rounded-2xl border p-6 sm:p-8 ${statusStyle.panelClass}`}>
            ...
            <div className="rounded-2xl border border-neutral-200 bg-white p-5">
              ...
              <div className="space-y-5">
                {/* BUTTON GROUP — What is your adjusted net income? */}
                <div>
                  <p className="mb-1 text-sm font-semibold text-neutral-800">What is your adjusted net income?</p>
                  <p className="mb-2 text-xs text-neutral-500">Salary + dividends + rental profit + other — before pension contributions</p>
                  <div className="flex flex-wrap gap-2">
                    
                    <button
                      type="button"
                      onClick={() => setAdjustedIncome(90000)}
                    ...
                    <button
                      type="button"
                      onClick={() => setAdjustedIncome(106000)}
                      className={`rounded-lg border-2 px-4 py-2.5 text-sm font-bold transition ${
                        adjustedIncome === 106000
                          ? "border-neutral-950 bg-neutral-950 text-white"
                          : "border-neutral-200 bg-white text-neutral-700 hover:border-neutral-950 hover:bg-neutral-950 hover:text-white"
                      }`}>
                      £100k–£112k
                    ...
                    <button
                      type="button"
                      onClick={() => setAdjustedIncome(118000)}
                      className={`rounded-lg border-2 px-4 py-2.5 text-sm font-bold transition ${
                        adjustedIncome === 118000
                          ? "border-neutral-950 bg-neutral-950 text-white"
                          : "border-neutral-200 bg-white text-neutral-700 hover:border-neutral-950 hover:bg-neutral-950 hover:text-white"
                      }`}>
                      £112k–£125k
                    ...
                    <button
                      type="button"
                      onClick={() => setAdjustedIncome(140000)}
                      className={`rounded-lg border-2 px-4 py-2.5 text-sm font-bold transition ${
                        adjustedIncome === 140000
                          ? "border-neutral-950 bg-neutral-950 text-white"
                          : "border-neutral-200 bg-white text-neutral-700 hover:border-neutral-950 hover:bg-neutral-950 hover:text-white"
                      }`}>
                      Over £125k
                ...
                {/* TWO-BUTTON TOGGLE — Are you currently making pension contributions to reduce your adjusted income? */}
                <div>
                  <p className="mb-1 text-sm font-semibold text-neutral-800">Are you currently making pension contributions to reduce your adjusted income?</p>
                  <p className="mb-2 text-xs text-neutral-500">Salary sacrifice or personal pension contributions both reduce adjusted net income</p>
                  <div className="flex gap-2">
                    
                    <button
                      type="button"
                      onClick={() => setMakesPensionContribs(true)}
                      className={`flex-1 rounded-lg border-2 py-3 text-sm font-bold transition ${
                        makesPensionContribs === true
                          ? "border-neutral-950 bg-neutral-950 text-white"
                          : "border-neutral-200 bg-white text-neutral-700 hover:border-neutral-950 hover:bg-neutral-950 hover:text-white"
                      }`}>
                      Yes — already contributing
                    </button>
                    <button
                      type="button"
                      onClick={() => setMakesPensionContribs(false)}
                      className={`flex-1 rounded-lg border-2 py-3 text-sm font-bold transition ${
                        makesPensionContribs === false
                          ? "border-neutral-950 bg-neutral-950 text-white"
                          : "border-neutral-200 bg-white text-neutral-700 hover:border-neutral-950 hover:bg-neutral-950 hover:text-white"
                      }`}>
                      No — not using pension to reduce income
                    </button>
                  </div>
                </div>
              </div>
