...
export default function DigitalLinkAuditorCalculator() {
  ...
  async function handleBracketSelect(index: number) {
    ...
    sessionStorage.setItem("digital-link-auditor_bracket", b.label);
    sessionStorage.setItem("digital-link-auditor_status",  b.status);
    ...
    try {
      ...
      if (data.id) {
        ...
        localStorage.setItem("digital-link-auditor_session_id", data.id);
      }
    } catch { /* non-blocking */ }
  }
  ...
  async function handleSaveEmail() {
    ...
    fetch("/api/save-email", {
      ...
      body: JSON.stringify({
        email,
        source:       "digital-link-auditor_result",
        country_code: "UK",
        site:         "taxchecknow",
        session_id:   sessionId ?? localStorage.getItem("digital-link-auditor_session_id") ?? "",
      }),
    }).catch(() => {});
    setEmailSent(true);
  }
      ...
      if (sid) {
        fetch("/api/decision-sessions", {
          ...
          body: JSON.stringify({
            ...
            product_key:           `uk_${effectiveTier}_digital_link_auditor`,
          ...
          <div ref={resultRef} className={`rounded-2xl border p-6 sm:p-8 ${statusStyle.panelClass}`}>
            ...
            <div className="mt-3 rounded-xl border border-neutral-200 bg-white px-4 py-3">
              ...
              <ul className="space-y-1 text-xs text-neutral-700">
                <li className="flex items-start gap-2"><span className="mt-0.5 text-emerald-600 shrink-0">✓</span><span>Your Digital Link Chain Audit — Map of your current VAT process and where digital links are broken.</span></li>
                <li className="flex items-start gap-2"><span className="mt-0.5 text-emerald-600 shrink-0">✓</span><span>Bridging Software Selection Guide — The right bridging tool for your spreadsheet setup.</span></li>
                <li className="flex items-start gap-2"><span className="mt-0.5 text-emerald-600 shrink-0">✓</span><span>Digital Link Evidence Pack — What to keep to evidence compliance if HMRC audits your process.</span></li>
                <li className="flex items-start gap-2"><span className="mt-0.5 text-emerald-600 shrink-0">✓</span><span>Accountant Brief — Digital Links — Questions for your accountant about VAT digital link compliance.</span></li>
                <li className="flex items-start gap-2"><span className="mt-0.5 text-emerald-600 shrink-0">✓</span><span>VAT Return Process Checklist — Step-by-step compliant VAT return process.</span></li>
              </ul>
            </div>
            ...
            <button
              ...
              Get My Digital Link Audit Pack →
            </button>
