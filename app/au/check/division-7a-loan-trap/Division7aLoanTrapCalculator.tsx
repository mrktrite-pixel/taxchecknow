...
const PRODUCTS: Record<PackTier, { name: string; tagline: string; value: string; cta: string }> = {
  67: {
    ...
    tagline: "Is your director loan about to become taxable income?",
    value:   "A personalised Div 7A audit — minimum repayment calculation, loan compliance check, and risk classification.",
    cta:     "Get My Div 7A Fix Plan — $67 →",
  },
  147: {
    name:    "Your Division 7A Loan Restructure System",
    tagline: "Restructure your loans before ATO deems them dividends",
    value:   "Full loan restructure plan, multi-year repayment strategy, and tax-efficient structuring to exit the Div 7A trap.",
    cta:     "Get My Div 7A System — $147 →",
  },
};
...
function recommendedTier(
  ...
  // loanStatus unsure, none, or missed → tier2. loanBalance over 150000 → tier2. Otherwise tier1.
    if ((loanBalance as number) > 150000 || loanStatus === "none" || loanStatus === "missed" || loanStatus === "unsure") return 147;
  return 67;
}
...
export default function Division7aLoanTrapCalculator() {
  ...
  const [loanBalance, setLoanBalance] = useState<number | string>(100000);
  const [loanStatus, setLoanStatus] = useState<number | string>("unsure");
  const [securedByProperty, setSecuredByProperty] = useState<boolean>(false);
  ...
  async function handleBracketSelect(index: number) {
    ...
    setLoanBalance(100000);
    setLoanStatus("unsure");
    setSecuredByProperty(false);
    setOverrideTier(null);
    setError("");
    // Save to sessionStorage for success page
    const b = BRACKETS[index];
    sessionStorage.setItem("division-7a-loan-trap_bracket", b.label);
    sessionStorage.setItem("division-7a-loan-trap_status",  b.status);
    ...
    try {
      ...
      if (data.id) {
        ...
        localStorage.setItem("division-7a-loan-trap_session_id", data.id);
      }
    } catch { /* non-blocking */ }
  }
  ...
  async function handleSaveEmail() {
    ...
    fetch("/api/save-email", {
      ...
      body: JSON.stringify({
        email,
        source:       "division-7a-loan-trap_result",
        country_code: "AU",
        site:         "taxchecknow",
        session_id:   sessionId ?? localStorage.getItem("division-7a-loan-trap_session_id") ?? "",
      }),
    }).catch(() => {});
    setEmailSent(true);
  }
  ...
  async function handleContinueToPayment() {
    ...
    sessionStorage.setItem("division-7a-loan-trap_answers", JSON.stringify(answers));
    sessionStorage.setItem("division-7a-loan-trap_loanBalance", String(loanBalance));
    sessionStorage.setItem("division-7a-loan-trap_loanStatus", String(loanStatus));
    sessionStorage.setItem("division-7a-loan-trap_securedByProperty", String(securedByProperty));
    const sid          = sessionId ?? localStorage.getItem("division-7a-loan-trap_session_id");
      ...
      if (sid) {
        fetch("/api/decision-sessions", {
          ...
          body: JSON.stringify({
            ...
            product_key:           `au_${effectiveTier}_division_7a_loan_trap`,
        ...
        <div className="rounded-2xl border border-neutral-200 bg-white p-6 shadow-sm sm:p-8">
          ...
          <div className="mt-4 rounded-xl border-2 border-amber-200 bg-amber-50 px-4 py-3">
            ...
            <p className="mt-1 text-xs leading-relaxed text-amber-900">
              The benchmark interest rate changes annually. For 2025/26 it is 8.27%. Your minimum repayment must be calculated using the correct rate for each year of the loan. Using the wrong rate means your repayments may be insufficient — creating a deemed dividend for the shortfall.
            </p>
          </div>
        </div>
          ...
          <div ref={resultRef} className={`rounded-2xl border p-6 sm:p-8 ${statusStyle.panelClass}`}>
            ...
            <div className="rounded-2xl border border-neutral-200 bg-white p-5">
              ...
              <div className="space-y-5">
                {/* BUTTON GROUP — What is the current loan balance from your company? */}
                <div>
                  <p className="mb-1 text-sm font-semibold text-neutral-800">What is the current loan balance from your company?</p>
                  <p className="mb-2 text-xs text-neutral-500">Total amount drawn from company and not repaid</p>
                  <div className="flex flex-wrap gap-2">
                    
                    <button
                      type="button"
                      onClick={() => setLoanBalance(25000)}
                ...
                {/* BUTTON GROUP — What is the status of the loan agreement? */}
                <div>
                  <p className="mb-1 text-sm font-semibold text-neutral-800">What is the status of the loan agreement?</p>
                  <p className="mb-2 text-xs text-neutral-500">Written Div 7A loan agreement in place?</p>
                  <div className="flex flex-wrap gap-2">
                    
                    <button
                      type="button"
                      onClick={() => setLoanStatus("compliant")}
                      className={`rounded-lg border-2 px-4 py-2.5 text-sm font-bold transition ${
                        loanStatus === "compliant"
                          ? "border-neutral-950 bg-neutral-950 text-white"
                          : "border-neutral-200 bg-white text-neutral-700 hover:border-neutral-950 hover:bg-neutral-950 hover:text-white"
                      }`}>
                    ...
                    <button
                      type="button"
                      onClick={() => setLoanStatus("unsure")}
                      className={`rounded-lg border-2 px-4 py-2.5 text-sm font-bold transition ${
                        loanStatus === "unsure"
                          ? "border-neutral-950 bg-neutral-950 text-white"
                          : "border-neutral-200 bg-white text-neutral-700 hover:border-neutral-950 hover:bg-neutral-950 hover:text-white"
                      }`}>
                      Not sure if agreement exists
                    </button>
                    <button
                      type="button"
                      onClick={() => setLoanStatus("none")}
                      className={`rounded-lg border-2 px-4 py-2.5 text-sm font-bold transition ${
                        loanStatus === "none"
                          ? "border-neutral-950 bg-neutral-950 text-white"
                          : "border-neutral-200 bg-white text-neutral-700 hover:border-neutral-950 hover:bg-neutral-950 hover:text-white"
                      }`}>
                      No agreement — informal drawings
                    </button>
                    <button
                      type="button"
                      onClick={() => setLoanStatus("missed")}
                      className={`rounded-lg border-2 px-4 py-2.5 text-sm font-bold transition ${
                        loanStatus === "missed"
                          ? "border-neutral-950 bg-neutral-950 text-white"
                          : "border-neutral-200 bg-white text-neutral-700 hover:border-neutral-950 hover:bg-neutral-950 hover:text-white"
                      }`}>
                      Agreement but missed repayments
                    </button>
                  </div>
                  
                </div>
                {/* TWO-BUTTON TOGGLE — Is the loan secured by a registered mortgage over real property? */}
                <div>
                  <p className="mb-1 text-sm font-semibold text-neutral-800">Is the loan secured by a registered mortgage over real property?</p>
                  <p className="mb-2 text-xs text-neutral-500">Secured loans can have 25-year repayment terms</p>
                  <div className="flex gap-2">
                    
                    <button
                      type="button"
                      onClick={() => setSecuredByProperty(false)}
                      className={`flex-1 rounded-lg border-2 py-3 text-sm font-bold transition ${
                        securedByProperty === false
                          ? "border-neutral-950 bg-neutral-950 text-white"
                          : "border-neutral-200 bg-white text-neutral-700 hover:border-neutral-950 hover:bg-neutral-950 hover:text-white"
                      }`}>
                      No — unsecured loan
                    ...
                    <button
                      type="button"
                      onClick={() => setSecuredByProperty(true)}
                      className={`flex-1 rounded-lg border-2 py-3 text-sm font-bold transition ${
                        securedByProperty === true
                          ? "border-neutral-950 bg-neutral-950 text-white"
                          : "border-neutral-200 bg-white text-neutral-700 hover:border-neutral-950 hover:bg-neutral-950 hover:text-white"
                      }`}>
                      Yes — secured by property
                    </button>
                  </div>
                </div>
              </div>
