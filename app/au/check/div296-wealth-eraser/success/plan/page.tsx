...
// Product: div296-wealth-eraser · Tier 2 Success Page
...
const FILES = [
  {
    "num": "01",
    "slug": "div296-01",
    "name": "Your Div 296 Exposure & Reset Decision",
    "desc": "Your exact Division 296 exposure and whether the cost-base reset is net beneficial for your fund.",
    "tier": 1
  },
  {
    "num": "02",
    "slug": "div296-02",
    "name": "SMSF Cost-Base Reset Decision Guide",
    "desc": "When to elect, when not to, and the all-or-nothing modelling framework.",
    "tier": 1
  },
  {
    "num": "03",
    "slug": "div296-03",
    "name": "Asset-Level Exposure Table",
    "desc": "Row-by-row modelling of each SMSF asset with and without the reset.",
    "tier": 1
  },
  {
    "num": "04",
    "slug": "div296-04",
    "name": "Valuation & Records Checklist",
    "desc": "Documentation required for the election and subsequent two-track record-keeping.",
    "tier": 1
  },
  {
    "num": "05",
    ...
    "desc": "4 questions to take to your SMSF accountant before 30 June 2026.",
    "tier": 1
  },
  {
    "num": "06",
    "slug": "div296-06",
    "name": "Full Cost-Base Reset Execution Plan",
    "desc": "Step-by-step execution for electing the cost-base reset without creating irreversible errors.",
    "tier": 2
  },
  {
    "num": "07",
    "slug": "div296-07",
    "name": "Pre-30-June Disposal Strategy",
    "desc": "How to optimise the all-or-nothing election by disposing of loss-position assets first.",
    "tier": 2
  },
  {
    "num": "08",
    "slug": "div296-08",
    "name": "Long-Term Two-Track Record-Keeping Model",
    "desc": "How to maintain the dual ordinary-CGT and Division 296 cost bases over 5-20 years.",
    "tier": 2
  }
];
...
export default function SuccessPlan() {
  ...
  async function generateAssessment(name: string) {
    ...
    try {
      ...
      const tsb_band = sessionStorage.getItem("div296-wealth-eraser_tsb_band") || "3m_to_10m";
      const embedded_gains_band = sessionStorage.getItem("div296-wealth-eraser_embedded_gains_band") || "500k_to_1m";
      const has_loss_assets = sessionStorage.getItem("div296-wealth-eraser_has_loss_assets") || "false";
      const div296_exposure = sessionStorage.getItem("div296-wealth-eraser_div296_exposure") || "50000";
      const avoidable_with_reset = sessionStorage.getItem("div296-wealth-eraser_avoidable_with_reset") || "30000";
      const election_recommended = sessionStorage.getItem("div296-wealth-eraser_election_recommended") || "true";
      const status = sessionStorage.getItem("div296-wealth-eraser_status") || "RESET RECOMMENDED";
      const tier = sessionStorage.getItem("div296-wealth-eraser_tier") || "147";

      // Check if we have any real inputs — sessionStorage may be empty after Stripe redirect
      const hasInputs = Object.values({
        "tsb_band": tsb_band,
        "embedded_gains_band": embedded_gains_band,
        "has_loss_assets": has_loss_assets,
        "div296_exposure": div296_exposure,
        "avoidable_with_reset": avoidable_with_reset,
        "election_recommended": election_recommended,
        "status": status,
        "tier": tier,
      }).some(v => v && v !== "3m_to_10m");
      ...
      const res = await fetch("/api/assess", {
        ...
        body: JSON.stringify({
          ...
          inputs: {
        "TSB band": tsb_band,
        "Embedded unrealised gains band": embedded_gains_band,
        "Has loss-position assets": has_loss_assets,
        "Estimated Div 296 tax exposure": div296_exposure,
        "Avoidable tax via reset election": avoidable_with_reset,
        "Cost-base reset recommended": election_recommended,
        "Verdict status": status,
        "Product tier purchased": tier,
          },
          fields: ["div296ExposureStatus","tsbBandAnalysis","embeddedGainsAnalysis","exposurePerAssetSummary","resetElectionRecommendation","avoidableTaxQuantum","lossAssetHandling","valuationRequirements","preJune30DisposalPlan","electionLodgementProcess","twoTrackRecordKeepingSetup","longTermProjectionModel","accountantImplementationChecklist"],
        }),
      });
      ...
      setAssessment({
        div296ExposureStatus: "Your personalised div296ExposureStatus is being prepared — please refresh in a moment.",
        tsbBandAnalysis: "Your personalised tsbBandAnalysis is being prepared — please refresh in a moment.",
        embeddedGainsAnalysis: "Your personalised embeddedGainsAnalysis is being prepared — please refresh in a moment.",
        exposurePerAssetSummary: "Your personalised exposurePerAssetSummary is being prepared — please refresh in a moment.",
        resetElectionRecommendation: "Your personalised resetElectionRecommendation is being prepared — please refresh in a moment.",
        avoidableTaxQuantum: "Your personalised avoidableTaxQuantum is being prepared — please refresh in a moment.",
        lossAssetHandling: "Your personalised lossAssetHandling is being prepared — please refresh in a moment.",
        valuationRequirements: "Your personalised valuationRequirements is being prepared — please refresh in a moment.",
        preJune30DisposalPlan: "Your personalised preJune30DisposalPlan is being prepared — please refresh in a moment.",
        electionLodgementProcess: "Your personalised electionLodgementProcess is being prepared — please refresh in a moment.",
        twoTrackRecordKeepingSetup: "Your personalised twoTrackRecordKeepingSetup is being prepared — please refresh in a moment.",
        longTermProjectionModel: "Your personalised longTermProjectionModel is being prepared — please refresh in a moment.",
  ...
  function handleCalendar() {
    ...
    const ics = [
      ...
      `X-WR-CALNAME:Division 296 Wealth Eraser Engine — Deadlines`,
      "BEGIN:VEVENT",
      `UID:div296-valuer-${Date.now()}@taxchecknow.com`,
      `DTSTART;VALUE=DATE:${"20260501"}`,
      `DTEND;VALUE=DATE:${"20260501"}`,
      `DTSTAMP:${now}`,
      "SUMMARY:Div 296 — book independent valuer",
      "DESCRIPTION:Illiquid assets (property, unlisted units) need qualified valuations dated near 30 June 2026.",
      "STATUS:CONFIRMED",
      "END:VEVENT",
      "BEGIN:VEVENT",
      `UID:div296-dispose-${Date.now()}@taxchecknow.com`,
      `DTSTART;VALUE=DATE:${"20260620"}`,
      `DTEND;VALUE=DATE:${"20260620"}`,
      `DTSTAMP:${now}`,
      "SUMMARY:Div 296 — loss-position asset disposals",
      "DESCRIPTION:Sell loss-position assets before 30 June 2026 to avoid all-or-nothing cost base lock-in.",
      "STATUS:CONFIRMED",
      "END:VEVENT",
      "BEGIN:VEVENT",
      `UID:div296-june30b-${Date.now()}@taxchecknow.com`,
      `DTSTART;VALUE=DATE:${"20260630"}`,
      `DTEND;VALUE=DATE:${"20260630"}`,
      `DTSTAMP:${now}`,
      "SUMMARY:Div 296 — valuation date",
      "DESCRIPTION:30 June 2026 market values lock in for cost-base reset election.",
      "STATUS:CONFIRMED",
      "END:VEVENT",
      "END:VCALENDAR",
    ].join("\r\n");
    ...
    a.href = url; a.download = "div296-wealth-eraser.ics";
  ...
  async function handleCopy() {
    ...
    await navigator.clipboard.writeText(
      `Your Div 296 Execution Pack — questions for my accountant:\n\n${text}\n\nTaxCheckNow · taxchecknow.com`
    );
    setCopied(true);
      ...
      <main className="mx-auto max-w-3xl space-y-5 px-6 py-8">
        ...
        <div className="print-section rounded-2xl border-2 border-emerald-500 bg-emerald-50 px-6 py-6">
          <p className="font-mono text-[10px] uppercase tracking-widest text-emerald-700">
            Payment confirmed · Your Div 296 Execution Pack · $147
          </p>
          <h1 className="mt-2 font-serif text-2xl font-bold text-neutral-950">
            {hi !== "there" ? `${hi}, here is your ` : "Your "}Your Div 296 Execution Pack
          </h1>
          ...
          <>
            ...
            <div className="print-section rounded-2xl border border-neutral-200 bg-white p-6">
              ...
              <div className="space-y-3">
                {(["div296ExposureStatus","tsbBandAnalysis","embeddedGainsAnalysis","exposurePerAssetSummary","resetElectionRecommendation","avoidableTaxQuantum"] as string[]).map(key => {
            ...
            <div className="print-section rounded-2xl border border-neutral-200 bg-white p-6">
              ...
              <div className="mb-4 space-y-2">
                ...
                <div className="flex items-center justify-between rounded-xl border border-neutral-100 bg-neutral-50 px-4 py-3">
                  <div>
                    <p className="text-sm font-semibold text-neutral-900">Div 296 — book independent valuer</p>
                    <p className="text-xs text-neutral-500">Illiquid assets (property, unlisted units) need qualified valuations dated near 30 June 2026.</p>
                  </div>
                ...
                <div className="flex items-center justify-between rounded-xl border border-neutral-100 bg-neutral-50 px-4 py-3">
                  <div>
                    <p className="text-sm font-semibold text-neutral-900">Div 296 — loss-position asset disposals</p>
                    <p className="text-xs text-neutral-500">Sell loss-position assets before 30 June 2026 to avoid all-or-nothing cost base lock-in.</p>
                  </div>
                  <span className="ml-3 shrink-0 font-mono text-xs font-bold text-neutral-500">
                    20 Jun 2026
                  </span>
                ...
                <div className="flex items-center justify-between rounded-xl border border-neutral-100 bg-neutral-50 px-4 py-3">
                  <div>
                    <p className="text-sm font-semibold text-neutral-900">Div 296 — valuation date</p>
                    <p className="text-xs text-neutral-500">30 June 2026 market values lock in for cost-base reset election.</p>
                  </div>
                  <span className="ml-3 shrink-0 font-mono text-xs font-bold text-neutral-500">
                    30 Jun 2026
                  </span>
              ...
              <div className="space-y-2">
                {FILES.map((f, i) => (
                  <div key={f.num} className={`flex items-center justify-between rounded-xl border px-4 py-3 ${
                    ...
                    <a href={`/files/au/div296-wealth-eraser/${f.slug}`}
            ...
            <div className="no-print rounded-2xl border border-neutral-100 bg-neutral-50 p-5">
              ...
              <p className="mb-1 text-sm font-bold text-neutral-950">Also check your death benefit exposure — Div 296 stacks with adult-children death tax.</p>
              <p className="mb-2 text-xs text-neutral-600">Division 296 drains while alive. Death benefit tax (17% to adult children) applies on the taxable component when you die. Both affect the same super — modelling them together is the only way to see what actually leaves your family.</p>
              <Link href="/au/check/super-death-tax-trap" className="font-mono text-xs font-bold text-neutral-700 underline hover:text-neutral-950">
