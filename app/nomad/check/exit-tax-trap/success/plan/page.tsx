...
// Product: exit-tax-trap · Tier 2 Success Page
...
const FILES = [
  {
    "num": "01",
    "slug": "ett-01",
    "name": "Your Exit Tax Risk Assessment",
    "desc": "Specific exit tax exposure for your departure country, asset mix, and timing.",
    "tier": 1
  },
  {
    "num": "02",
    "slug": "ett-02",
    "name": "Country-Specific Exit Tax Rules",
    "desc": "Detailed comparison of exit tax rules across Canada / AU / UK / US / NZ.",
    "tier": 1
  },
  {
    "num": "03",
    ...
    "desc": "How different asset types are treated at departure in each country.",
    "tier": 1
  },
  {
    "num": "04",
    "slug": "ett-04",
    "name": "Voluntary Disclosure Opportunity Check",
    "desc": "Can you retroactively fix an unfiled or incorrect departure return?",
    "tier": 1
  },
  {
    "num": "05",
    ...
    "desc": "Questions to ask your cross-border tax advisor before or after departure.",
    "tier": 1
  },
  {
    "num": "06",
    ...
    "desc": "Country-specific pre-departure planning sequence to minimise exit tax.",
    "tier": 2
  },
  {
    "num": "07",
    "slug": "ett-07",
    "name": "Deemed Disposition Management Strategy",
    "desc": "Canada-specific: managing the deemed disposition tax with T1244 deferral and security.",
    "tier": 2
  },
  {
    "num": "08",
    "slug": "ett-08",
    "name": "Multi-Country Exit Sequencing Plan",
    "desc": "For people leaving one country and arriving in another — the order of operations matters.",
    "tier": 2
  }
];
...
export default function SuccessPlan() {
  ...
  async function generateAssessment(name: string) {
    ...
    try {
      ...
      const departure_country = sessionStorage.getItem("exit-tax-trap_departure_country") || "Canada";
      const departure_timing = sessionStorage.getItem("exit-tax-trap_departure_timing") || "within_2_years";
      const asset_types = sessionStorage.getItem("exit-tax-trap_asset_types") || "shares_property";
      const advice_taken = sessionStorage.getItem("exit-tax-trap_advice_taken") || "no";
      const assets_sold = sessionStorage.getItem("exit-tax-trap_assets_sold") || "no";
      const exit_tax_triggered = sessionStorage.getItem("exit-tax-trap_exit_tax_triggered") || "yes";
      const status = sessionStorage.getItem("exit-tax-trap_status") || "EXIT TAX EXPOSURE";
      const tier = sessionStorage.getItem("exit-tax-trap_tier") || "147";
      ...
      const hasInputs = Object.values({
        ...
        "advice_taken": advice_taken,
        "assets_sold": assets_sold,
        "exit_tax_triggered": exit_tax_triggered,
        "status": status,
        "tier": tier,
      }).some(v => v && v !== "Canada");
      ...
      const res = await fetch("/api/assess", {
        ...
        body: JSON.stringify({
          ...
          market:     "Global (cross-border departure)",
          authority:  "National tax authorities (CRA / ATO / HMRC / IRD NZ / IRS)",
          tier:       2,
          name,
          inputs: {
        "Departure country": departure_country,
        "Departure timing": departure_timing,
        "Asset types at departure": asset_types,
        "Tax advice taken": advice_taken,
        "Assets sold since departure": assets_sold,
        "Exit tax triggered": exit_tax_triggered,
        "Verdict status": status,
        "Tier purchased": tier,
          },
          fields: ["exitTaxRuleApplicable","departureCountryAnalysis","deemedDispositionExposure","assetRiskRanking","timingImpact","voluntaryDisclosureOpportunity","filingObligations","immediateActions","preDepartureOptimisationStrategy","deferralElectionAnalysis","assetDisposalSequencing","multiCountryExitPlan","auditDefenceDocumentation"],
        }),
      });
      ...
      setAssessment({
        exitTaxRuleApplicable: "Your personalised exitTaxRuleApplicable is being prepared — please refresh in a moment.",
        departureCountryAnalysis: "Your personalised departureCountryAnalysis is being prepared — please refresh in a moment.",
        deemedDispositionExposure: "Your personalised deemedDispositionExposure is being prepared — please refresh in a moment.",
        assetRiskRanking: "Your personalised assetRiskRanking is being prepared — please refresh in a moment.",
        timingImpact: "Your personalised timingImpact is being prepared — please refresh in a moment.",
        voluntaryDisclosureOpportunity: "Your personalised voluntaryDisclosureOpportunity is being prepared — please refresh in a moment.",
        ...
        preDepartureOptimisationStrategy: "Your personalised preDepartureOptimisationStrategy is being prepared — please refresh in a moment.",
        deferralElectionAnalysis: "Your personalised deferralElectionAnalysis is being prepared — please refresh in a moment.",
        assetDisposalSequencing: "Your personalised assetDisposalSequencing is being prepared — please refresh in a moment.",
        multiCountryExitPlan: "Your personalised multiCountryExitPlan is being prepared — please refresh in a moment.",
        auditDefenceDocumentation: "Your personalised auditDefenceDocumentation is being prepared — please refresh in a moment.",
        accountantQuestions: [
          "What is my exact National tax authorities (CRA / ATO / HMRC / IRD NZ / IRS) position based on my answers?",
  ...
  function handleCalendar() {
    ...
    const ics = [
      ...
      `X-WR-CALNAME:Exit Tax Trap Auditor — Deadlines`,
      "BEGIN:VEVENT",
      `UID:ett-review-${Date.now()}@taxchecknow.com`,
      `DTSTART;VALUE=DATE:${relativeDate(90)}`,
      `DTEND;VALUE=DATE:${relativeDate(90)}`,
      `DTSTAMP:${now}`,
      "SUMMARY:Pre-departure — 90-day review",
      "DESCRIPTION:Review asset positions, planning levers, and departure timing 90 days before intended departure.",
      "STATUS:CONFIRMED",
      "END:VEVENT",
      "BEGIN:VEVENT",
      `UID:ett-exec-${Date.now()}@taxchecknow.com`,
      `DTSTART;VALUE=DATE:${relativeDate(30)}`,
      `DTEND;VALUE=DATE:${relativeDate(30)}`,
      `DTSTAMP:${now}`,
      "SUMMARY:Pre-departure — 30-day execution",
      "DESCRIPTION:Execute planning trades, file elections, establish new country residency 30 days before departure.",
      "STATUS:CONFIRMED",
      "END:VEVENT",
      "BEGIN:VEVENT",
      `UID:ett-ca-${Date.now()}@taxchecknow.com`,
      `DTSTART;VALUE=DATE:${"20270430"}`,
      `DTEND;VALUE=DATE:${"20270430"}`,
      `DTSTAMP:${now}`,
      "SUMMARY:Canada departure return — 30 April",
      "DESCRIPTION:T1 return with Schedule 3 + T1161 + T1244 election.",
      "STATUS:CONFIRMED",
      "END:VEVENT",
      "BEGIN:VEVENT",
      `UID:ett-au-${Date.now()}@taxchecknow.com`,
      `DTSTART;VALUE=DATE:${"20261031"}`,
      `DTEND;VALUE=DATE:${"20261031"}`,
      `DTSTAMP:${now}`,
      "SUMMARY:AU departure statement + return by 31 Oct",
      "DESCRIPTION:ATO leaving statement + full year individual return.",
      "STATUS:CONFIRMED",
      "END:VEVENT",
      "BEGIN:VEVENT",
      `UID:ett-us-${Date.now()}@taxchecknow.com`,
      `DTSTART;VALUE=DATE:${"20270415"}`,
      `DTEND;VALUE=DATE:${"20270415"}`,
      `DTSTAMP:${now}`,
      "SUMMARY:US final 1040 + 8854",
      "DESCRIPTION:Final 1040 + expatriation statement.",
      "STATUS:CONFIRMED",
      "END:VEVENT",
      "END:VCALENDAR",
    ].join("\r\n");
    ...
    a.href = url; a.download = "exit-tax-trap.ics";
  ...
  async function handleCopy() {
    ...
    await navigator.clipboard.writeText(
      `Your Exit Tax Strategy — questions for my accountant:\n\n${text}\n\nTaxCheckNow · taxchecknow.com`
    );
    setCopied(true);
      ...
      <main className="mx-auto max-w-3xl space-y-5 px-6 py-8">
        ...
        <div className="print-section rounded-2xl border-2 border-emerald-500 bg-emerald-50 px-6 py-6">
          <p className="font-mono text-[10px] uppercase tracking-widest text-emerald-700">
            Payment confirmed · Your Exit Tax Strategy · $147
          </p>
          <h1 className="mt-2 font-serif text-2xl font-bold text-neutral-950">
            {hi !== "there" ? `${hi}, here is your ` : "Your "}Your Exit Tax Strategy
          </h1>
        ...
        {loading && (
          <div className="rounded-2xl border border-neutral-200 bg-white p-10 text-center">
            ...
            <p className="mt-1 text-xs text-neutral-400">Analysing your answers against National tax authorities (CRA / ATO / HMRC / IRD NZ / IRS) rules</p>
          </div>
        )}
          ...
          <>
            ...
            <div className="print-section rounded-2xl border border-neutral-200 bg-white p-6">
              <p className="mb-1 font-mono text-[10px] uppercase tracking-widest text-neutral-400">
                Your Global (cross-border departure) National tax authorities (CRA / ATO / HMRC / IRD NZ / IRS) position
              </p>
              <h2 className="mb-4 font-serif text-xl font-bold text-neutral-950">
                What this means for {greeting}
              </h2>
              <div className="space-y-3">
                {(["exitTaxRuleApplicable","departureCountryAnalysis","deemedDispositionExposure","assetRiskRanking","timingImpact","voluntaryDisclosureOpportunity"] as string[]).map(key => {
            ...
            <div className="print-section rounded-2xl border border-neutral-200 bg-white p-6">
              ...
              <div className="mb-4 space-y-2">
                ...
                <div className="flex items-center justify-between rounded-xl border border-neutral-100 bg-neutral-50 px-4 py-3">
                  <div>
                    <p className="text-sm font-semibold text-neutral-900">Pre-departure — 90-day review</p>
                    <p className="text-xs text-neutral-500">Review asset positions, planning levers, and departure timing 90 days before intended departure.</p>
                  </div>
                ...
                <div className="flex items-center justify-between rounded-xl border border-neutral-100 bg-neutral-50 px-4 py-3">
                  <div>
                    <p className="text-sm font-semibold text-neutral-900">Pre-departure — 30-day execution</p>
                    <p className="text-xs text-neutral-500">Execute planning trades, file elections, establish new country residency 30 days before departure.</p>
                  </div>
                  <span className="ml-3 shrink-0 font-mono text-xs font-bold text-neutral-500">
                    This week
                  </span>
                ...
                <div className="flex items-center justify-between rounded-xl border border-neutral-100 bg-neutral-50 px-4 py-3">
                  <div>
                    <p className="text-sm font-semibold text-neutral-900">Canada departure return — 30 April</p>
                    <p className="text-xs text-neutral-500">T1 return with Schedule 3 + T1161 + T1244 election.</p>
                  </div>
                  <span className="ml-3 shrink-0 font-mono text-xs font-bold text-neutral-500">
                    30 Apr 2027
                  </span>
                ...
                <div className="flex items-center justify-between rounded-xl border border-neutral-100 bg-neutral-50 px-4 py-3">
                  <div>
                    <p className="text-sm font-semibold text-neutral-900">AU departure statement + return by 31 Oct</p>
                    <p className="text-xs text-neutral-500">ATO leaving statement + full year individual return.</p>
                  </div>
                  <span className="ml-3 shrink-0 font-mono text-xs font-bold text-neutral-500">
                    31 Oct 2026
                  </span>
                ...
                <div className="flex items-center justify-between rounded-xl border border-neutral-100 bg-neutral-50 px-4 py-3">
                  <div>
                    <p className="text-sm font-semibold text-neutral-900">US final 1040 + 8854</p>
                    <p className="text-xs text-neutral-500">Final 1040 + expatriation statement.</p>
                  </div>
                  <span className="ml-3 shrink-0 font-mono text-xs font-bold text-neutral-500">
                    15 Apr 2027
                  </span>
            ...
            <div className="print-section rounded-2xl border border-neutral-200 bg-white p-6">
              ...
              <p className="mb-4 text-sm text-neutral-500">
                Each document is built around your specific National tax authorities (CRA / ATO / HMRC / IRD NZ / IRS) position.
              ...
              <div className="space-y-2">
                {FILES.map((f, i) => (
                  <div key={f.num} className={`flex items-center justify-between rounded-xl border px-4 py-3 ${
                    ...
                    <a href={`/files/global/exit-tax-trap/${f.slug}`}
            ...
            <div className="no-print rounded-2xl border border-neutral-100 bg-neutral-50 p-5">
              ...
              <p className="mb-1 text-sm font-bold text-neutral-950">Before exit tax: confirm your residency status first</p>
              <p className="mb-2 text-xs text-neutral-600">Whether exit tax triggers at all depends on whether you have actually ceased tax residency under the departure country's specific test. Start with the 183-Day Rule Reality Check to confirm residency status.</p>
        ...
        <div className="rounded-xl bg-neutral-100 px-5 py-4">
          <p className="text-xs leading-relaxed text-neutral-500">
            ...
            Always consult a qualified Global (cross-border departure) tax adviser before making financial decisions.
            Based on National tax authorities (CRA / ATO / HMRC / IRD NZ / IRS) guidance April 2026.{" "}
            <a href="https://www.canada.ca/en/revenue-agency/services/tax/international-non-residents/individuals-leaving-or-entering-canada-establishing-residency/leaving-canada-emigrants.html" target="_blank" rel="noopener noreferrer" className="underline">CRA — Leaving Canada (emigrants)</a> · <a href="https://www.canada.ca/en/revenue-agency/services/tax/international-non-residents/information-been-moved/disposing-property.html" target="_blank" rel="noopener noreferrer" className="underline">CRA — Deemed disposition on emigration</a>
          </p>
        </div>
