...
export default function SideHustleCheckerCalculator() {
  ...
  async function handleBracketSelect(index: number) {
    ...
    sessionStorage.setItem("side-hustle-checker_bracket", b.label);
    sessionStorage.setItem("side-hustle-checker_status",  b.status);
    ...
    try {
      ...
      if (data.id) {
        ...
        localStorage.setItem("side-hustle-checker_session_id", data.id);
      }
    } catch { /* non-blocking */ }
  }
  ...
  async function handleSaveEmail() {
    ...
    fetch("/api/save-email", {
      ...
      body: JSON.stringify({
        email,
        source:       "side-hustle-checker_result",
        country_code: "UK",
        site:         "taxchecknow",
        session_id:   sessionId ?? localStorage.getItem("side-hustle-checker_session_id") ?? "",
      }),
    }).catch(() => {});
    setEmailSent(true);
  }
      ...
      if (sid) {
        fetch("/api/decision-sessions", {
          ...
          body: JSON.stringify({
            ...
            product_key:           `uk_${effectiveTier}_side_hustle_checker`,
          ...
          <div ref={resultRef} className={`rounded-2xl border p-6 sm:p-8 ${statusStyle.panelClass}`}>
            ...
            <div className="mt-3 rounded-xl border border-neutral-200 bg-white px-4 py-3">
              ...
              <ul className="space-y-1 text-xs text-neutral-700">
                <li className="flex items-start gap-2"><span className="mt-0.5 text-emerald-600 shrink-0">✓</span><span>Your Side Hustle Tax Verdict — Your exact declaration position — what you owe and what you need to do.</span></li>
                <li className="flex items-start gap-2"><span className="mt-0.5 text-emerald-600 shrink-0">✓</span><span>Self Assessment Registration Guide — Step-by-step self assessment registration for side hustlers.</span></li>
                <li className="flex items-start gap-2"><span className="mt-0.5 text-emerald-600 shrink-0">✓</span><span>Trading Allowance vs Expenses Calculator — Which method saves you more tax — allowance or actual costs?</span></li>
                <li className="flex items-start gap-2"><span className="mt-0.5 text-emerald-600 shrink-0">✓</span><span>Platform Income Record Template — Monthly income tracker for all platform earnings.</span></li>
                <li className="flex items-start gap-2"><span className="mt-0.5 text-emerald-600 shrink-0">✓</span><span>Accountant Brief — Side Hustle — Questions to take to your accountant.</span></li>
              </ul>
            </div>
