...
// ── CURRENCY HELPER ───────────────────────────────────────────────────────────
function currencySymbol(config: ProductConfig): string {
...
function buildSuccessPage(
  ...
  const sym          = currencySymbol(config);
...
export default function Success${isTier2 ? "Plan" : "Assess"}() {
  ...
  async function generateAssessment(name: string) {
    ...
    try {
${promptFieldsCode}
      const prompt = \`You are a ${config.market} ${config.authority} tax expert. Write a personalised ${isTier2 ? "action plan" : "assessment"} for \${name !== "your" ? name : "this taxpayer"}.

Their answers:
${promptDataCode}

Rules:
- Use ${config.market} tax terminology throughout
- Reference ${config.authority} rules, thresholds, and legislation
...
Required fields:
${assessFields.map(f => `"${f}": "personalised value based on their answers"`).join(",\n")}
  ...
  function buildFallback(name: string): Assessment {
    ...
    ${assessFields.map(f =>
      f === "accountantQuestions" || f === "actions" || f === "weekPlan"
        ? `result["${f}"] = [];`
        : `result["${f}"] = "Preparing " + n + "'s ${f.replace(/_/g," ")}...";`
    ).join("\n    ")}
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="font-mono text-[10px] uppercase tracking-widest text-emerald-700">
                Payment confirmed · ${packName} · ${sym}${price}
              </p>
          ...
          <>
            ...
            <div className="print-section rounded-2xl border border-neutral-200 bg-white p-6">
              <p className="mb-3 font-mono text-[10px] uppercase tracking-widest text-neutral-400">
                Your ${config.market} tax position
              </p>
...
${isTier2 ? `
            ...
            {Array.isArray(assessment.actions) && (assessment.actions as { title: string; deadline: string; steps: string[] }[]).length > 0 && (
              <div className="print-section rounded-2xl border border-neutral-200 bg-white p-6">
                ...
                <h2 className="mb-4 font-serif text-xl font-bold text-neutral-950">Actions to take before ${config.deadline.display}</h2>
                ...
                <div className="mb-3 flex items-start justify-between gap-4">
                  <div>
                    ...
                    <p className="mt-0.5 text-xs text-blue-600">
                      Full brief in <a href="/files/${config.country}/${config.id}/${config.files.find(f => f.num === "05")?.slug ?? ""}"
              ...
              <div className="space-y-2">
                {FILES.map(f => (
                  <div key={f.num} className={\`flex items-center justify-between rounded-xl border px-4 py-3 \${
                    ...
                    <a href={\`/files/${config.country}/${config.id}/\${f.slug}\`} target="_blank" rel="noopener noreferrer"
              ...
              <div className="rounded-xl border border-neutral-800 bg-neutral-900 px-4 py-3">
                ...
                <p className="mt-2 text-xs text-neutral-500">
                  ...
                  <a href="${config.sources[0]?.url}" target="_blank" rel="noopener noreferrer"
                    className="underline hover:text-neutral-400 transition">${config.authority} source</a>
                </p>
              </div>
            </div>
${!isTier2 ? `
            ...
            <div className="no-print rounded-2xl border border-neutral-200 bg-neutral-50 p-6">
              ...
              <p className="mb-1 font-serif text-lg font-bold text-neutral-950">Upgrade to ${config.tier2.name}</p>
              <p className="mb-3 text-sm leading-relaxed text-neutral-600">${config.tier2.value}</p>
              <Link href="/${config.slug}"
                className="font-mono text-xs font-bold text-neutral-700 underline transition hover:text-neutral-950">
                Get ${config.tier2.name} — ${sym}${config.tier2.price} →
              </Link>
            </div>` : ""}
          </>
        )}
