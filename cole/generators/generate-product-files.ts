...
export function generateProductFile(
  ...
  const isLastInTier = file.tier === 1
    ? file.num === "05"
    : file.num === "08";
    ...
    <div className="min-h-screen bg-white">
      ...
      <nav className="no-print sticky top-0 z-50 border-b border-neutral-200 bg-white px-4 py-3">
        <div className="mx-auto flex max-w-3xl items-center justify-between">
          ...
          <div className="flex items-center gap-4 text-sm text-neutral-500">
            <span>United Kingdom · ${config.name}</span>
            <button
      ...
      <main className="mx-auto max-w-3xl px-4 py-8">
        ...
        <div
          ...
          dangerouslySetInnerHTML={{ __html: \`${escapeTemplateString(file.content)}\` }}
        />
        ...
        <div className="no-print mt-12 border-t border-neutral-200 pt-6">
          ...
          <div className="space-y-2">
            ...
            <a href="/files/${config.country}/${config.id}/${f.slug}"
              ...
              <div>
                <span className="text-xs font-mono font-bold mr-2">${f.num}</span>
                <span className="text-sm font-semibold">${f.name}</span>
                ${f.tier === 2 ? `<span className="ml-2 text-[9px] font-mono uppercase tracking-widest text-blue-500">Plan only</span>` : ''}
              </div>
              ${"${\"" + f.num + "\" === \"" + file.num + "\" ? <span className=\"text-xs text-neutral-400\">You are here</span> : <span className=\"text-xs\">Open →</span>}"}
            </a>`).join('\n            ')}
          </div>
        </div>
...
// ── GENERATE ALL FILES FOR A PRODUCT ─────────────────────────────────────────
